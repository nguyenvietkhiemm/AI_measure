...
def remove_outliers_iqr(df, column):
    ...
    Q3 = df_clean[column].quantile(0.99)
    IQR = Q3 - Q1
    lower_bound = Q1 - 1.5 * IQR
    upper_bound = Q3 + 1.5 * IQR

    # Sử dụng np.logical_and để kết hợp điều kiện
    return df_clean[np.logical_and(df_clean[column] >= lower_bound, df_clean[column] <= upper_bound)]
